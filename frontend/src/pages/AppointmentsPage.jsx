...
function ApptRow({ row, idx, canEdit, onView, onEdit, onDelete, onStatusChange }) {
  ...
  return (
    <PageWrapper title="Appointments" subtitle={`${total} total appointments`}
            ...
            <tbody>
              {loading ? Array.from({length:5}).map((_,i)=>(
                ...
                <ApptRow key={row.id} row={row} idx={idx} canEdit={canEdit}
                  ...
                  onStatusChange={v=>handleStatusChange(row.id,v)} />
              ))}
            </tbody>
          </table>
      ...
      <Drawer open={showDetail} onClose={()=>setShowDetail(false)} title="Appointment Details"
        footer={canEdit&&detailItem?.status!=='completed'&&detailItem?.status!=='cancelled'&&(
          <Button variant="primary" onClick={()=>{setShowDetail(false);openEdit(detailItem);}} style={{ display:'flex', alignItems:'center', gap:6 }}><IconEdit /> Edit Appointment</Button>
        )}>
        {detailItem && (
